...
from models import Staffs, WorkOrders
...
def get_current_user(user_id: str, db: Session = Depends(get_db)):
    ...
    if not user_id.isupper():
        work_order = db.query(WorkOrders).filter(WorkOrders.order_number == user_id).first()
        if not work_order:
            raise HTTPException(status_code=404, detail="User not found: Cannot create workorder")
        return [work_order.order_number]
    else:
...
async def get_workorders_requests(user: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if len(user) == 2 and user[1] == 'maid supervisor':
        ...
        # For Workorders
        work_order = db.query(WorkOrders).filter(WorkOrders.order_number == user[0]).first()

        if work_order.checkout_date > datetime.now():
...
def create_technician_request(
    ...
    if len(user) == 2 and user[1]=='supervisor':
        ...
        # For Guests
        guest = db.query(WorkOrders).filter(WorkOrders.order_number == user[0]).first()
        if not guest:
